<?php
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# NO CHECKED-IN PROTOBUF GENCODE
# source: centrifugo/api/v1/api.proto

namespace RoadRunner\Centrifugal\API\DTO\V1;

use Google\Protobuf\Internal\GPBType;
use Google\Protobuf\Internal\GPBUtil;
use Google\Protobuf\RepeatedField;

/**
 * Generated from protobuf message <code>centrifugal.centrifugo.api.DeviceFilter</code>
 */
class DeviceFilter extends \Google\Protobuf\Internal\Message
{
    /**
     * Generated from protobuf field <code>repeated string ids = 1 [json_name = "ids"];</code>
     */
    private $ids;
    /**
     * Generated from protobuf field <code>repeated string users = 2 [json_name = "users"];</code>
     */
    private $users;
    /**
     * Generated from protobuf field <code>repeated string topics = 3 [json_name = "topics"];</code>
     */
    private $topics;
    /**
     * Generated from protobuf field <code>repeated string providers = 4 [json_name = "providers"];</code>
     */
    private $providers;
    /**
     * Generated from protobuf field <code>repeated string platforms = 5 [json_name = "platforms"];</code>
     */
    private $platforms;

    /**
     * Constructor.
     *
     * @param array $data {
     *     Optional. Data for populating the Message object.
     *
     *     @type string[] $ids
     *     @type string[] $users
     *     @type string[] $topics
     *     @type string[] $providers
     *     @type string[] $platforms
     * }
     */
    public function __construct($data = NULL) {
        \RoadRunner\Centrifugal\API\DTO\V1\GPBMetadata\Api::initOnce();
        parent::__construct($data);
    }

    /**
     * Generated from protobuf field <code>repeated string ids = 1 [json_name = "ids"];</code>
     * @return RepeatedField<string>
     */
    public function getIds()
    {
        return $this->ids;
    }

    /**
     * Generated from protobuf field <code>repeated string ids = 1 [json_name = "ids"];</code>
     * @param string[] $var
     * @return $this
     */
    public function setIds(array|RepeatedField $var)
    {
        $arr = GPBUtil::checkRepeatedField($var, \Google\Protobuf\Internal\GPBType::STRING);
        $this->ids = $arr;

        return $this;
    }

    /**
     * Generated from protobuf field <code>repeated string users = 2 [json_name = "users"];</code>
     * @return RepeatedField<string>
     */
    public function getUsers()
    {
        return $this->users;
    }

    /**
     * Generated from protobuf field <code>repeated string users = 2 [json_name = "users"];</code>
     * @param string[] $var
     * @return $this
     */
    public function setUsers(array|RepeatedField $var)
    {
        $arr = GPBUtil::checkRepeatedField($var, \Google\Protobuf\Internal\GPBType::STRING);
        $this->users = $arr;

        return $this;
    }

    /**
     * Generated from protobuf field <code>repeated string topics = 3 [json_name = "topics"];</code>
     * @return RepeatedField<string>
     */
    public function getTopics()
    {
        return $this->topics;
    }

    /**
     * Generated from protobuf field <code>repeated string topics = 3 [json_name = "topics"];</code>
     * @param string[] $var
     * @return $this
     */
    public function setTopics(array|RepeatedField $var)
    {
        $arr = GPBUtil::checkRepeatedField($var, \Google\Protobuf\Internal\GPBType::STRING);
        $this->topics = $arr;

        return $this;
    }

    /**
     * Generated from protobuf field <code>repeated string providers = 4 [json_name = "providers"];</code>
     * @return RepeatedField<string>
     */
    public function getProviders()
    {
        return $this->providers;
    }

    /**
     * Generated from protobuf field <code>repeated string providers = 4 [json_name = "providers"];</code>
     * @param string[] $var
     * @return $this
     */
    public function setProviders(array|RepeatedField $var)
    {
        $arr = GPBUtil::checkRepeatedField($var, \Google\Protobuf\Internal\GPBType::STRING);
        $this->providers = $arr;

        return $this;
    }

    /**
     * Generated from protobuf field <code>repeated string platforms = 5 [json_name = "platforms"];</code>
     * @return RepeatedField<string>
     */
    public function getPlatforms()
    {
        return $this->platforms;
    }

    /**
     * Generated from protobuf field <code>repeated string platforms = 5 [json_name = "platforms"];</code>
     * @param string[] $var
     * @return $this
     */
    public function setPlatforms(array|RepeatedField $var)
    {
        $arr = GPBUtil::checkRepeatedField($var, \Google\Protobuf\Internal\GPBType::STRING);
        $this->platforms = $arr;

        return $this;
    }

}

