<?php
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# NO CHECKED-IN PROTOBUF GENCODE
# source: temporal/api/errordetails/v1/message.proto

namespace Temporal\Api\Errordetails\V1;

use Google\Protobuf\Internal\GPBType;
use Google\Protobuf\Internal\GPBUtil;
use Google\Protobuf\RepeatedField;

/**
 * Generated from protobuf message <code>temporal.api.errordetails.v1.ResourceExhaustedFailure</code>
 */
class ResourceExhaustedFailure extends \Google\Protobuf\Internal\Message
{
    /**
     * Generated from protobuf field <code>.temporal.api.enums.v1.ResourceExhaustedCause cause = 1 [json_name = "cause"];</code>
     */
    protected $cause = 0;
    /**
     * Generated from protobuf field <code>.temporal.api.enums.v1.ResourceExhaustedScope scope = 2 [json_name = "scope"];</code>
     */
    protected $scope = 0;

    /**
     * Constructor.
     *
     * @param array $data {
     *     Optional. Data for populating the Message object.
     *
     *     @type int $cause
     *     @type int $scope
     * }
     */
    public function __construct($data = NULL) {
        \GPBMetadata\Temporal\Api\Errordetails\V1\Message::initOnce();
        parent::__construct($data);
    }

    /**
     * Generated from protobuf field <code>.temporal.api.enums.v1.ResourceExhaustedCause cause = 1 [json_name = "cause"];</code>
     * @return int one of the values in {@see \Temporal\Api\Enums\V1\ResourceExhaustedCause}
     */
    public function getCause()
    {
        return $this->cause;
    }

    /**
     * Generated from protobuf field <code>.temporal.api.enums.v1.ResourceExhaustedCause cause = 1 [json_name = "cause"];</code>
     * @param int $var one of the values in {@see \Temporal\Api\Enums\V1\ResourceExhaustedCause}
     * @return $this
     */
    public function setCause(int $var)
    {
        GPBUtil::checkEnum($var, \Temporal\Api\Enums\V1\ResourceExhaustedCause::class);
        $this->cause = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>.temporal.api.enums.v1.ResourceExhaustedScope scope = 2 [json_name = "scope"];</code>
     * @return int one of the values in {@see \Temporal\Api\Enums\V1\ResourceExhaustedScope}
     */
    public function getScope()
    {
        return $this->scope;
    }

    /**
     * Generated from protobuf field <code>.temporal.api.enums.v1.ResourceExhaustedScope scope = 2 [json_name = "scope"];</code>
     * @param int $var one of the values in {@see \Temporal\Api\Enums\V1\ResourceExhaustedScope}
     * @return $this
     */
    public function setScope(int $var)
    {
        GPBUtil::checkEnum($var, \Temporal\Api\Enums\V1\ResourceExhaustedScope::class);
        $this->scope = $var;

        return $this;
    }

}

