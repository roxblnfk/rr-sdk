<?php
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# NO CHECKED-IN PROTOBUF GENCODE
# source: temporal/api/nexus/v1/message.proto

namespace Temporal\Api\Nexus\V1;

use Google\Protobuf\Internal\GPBType;
use Google\Protobuf\Internal\GPBUtil;
use Google\Protobuf\RepeatedField;

/**
 * A request to start an operation.
 *
 * Generated from protobuf message <code>temporal.api.nexus.v1.StartOperationRequest</code>
 */
class StartOperationRequest extends \Google\Protobuf\Internal\Message
{
    /**
     * Name of service to start the operation in.
     *
     * Generated from protobuf field <code>string service = 1 [json_name = "service"];</code>
     */
    protected $service = '';
    /**
     * Type of operation to start.
     *
     * Generated from protobuf field <code>string operation = 2 [json_name = "operation"];</code>
     */
    protected $operation = '';
    /**
     * A request ID that can be used as an idempotentency key.
     *
     * Generated from protobuf field <code>string request_id = 3 [json_name = "requestId"];</code>
     */
    protected $request_id = '';
    /**
     * Callback URL to call upon completion if the started operation is async.
     *
     * Generated from protobuf field <code>string callback = 4 [json_name = "callback"];</code>
     */
    protected $callback = '';
    /**
     * Full request body from the incoming HTTP request.
     *
     * Generated from protobuf field <code>.temporal.api.common.v1.Payload payload = 5 [json_name = "payload"];</code>
     */
    protected $payload = null;
    /**
     * Header that is expected to be attached to the callback request when the operation completes.
     *
     * Generated from protobuf field <code>map<string, string> callback_header = 6 [json_name = "callbackHeader"];</code>
     */
    private $callback_header;
    /**
     * Links contain caller information and can be attached to the operations started by the handler.
     *
     * Generated from protobuf field <code>repeated .temporal.api.nexus.v1.Link links = 7 [json_name = "links"];</code>
     */
    private $links;

    /**
     * Constructor.
     *
     * @param array $data {
     *     Optional. Data for populating the Message object.
     *
     *     @type string $service
     *           Name of service to start the operation in.
     *     @type string $operation
     *           Type of operation to start.
     *     @type string $request_id
     *           A request ID that can be used as an idempotentency key.
     *     @type string $callback
     *           Callback URL to call upon completion if the started operation is async.
     *     @type \Temporal\Api\Common\V1\Payload $payload
     *           Full request body from the incoming HTTP request.
     *     @type array|\Google\Protobuf\Internal\MapField $callback_header
     *           Header that is expected to be attached to the callback request when the operation completes.
     *     @type \Temporal\Api\Nexus\V1\Link[] $links
     *           Links contain caller information and can be attached to the operations started by the handler.
     * }
     */
    public function __construct($data = NULL) {
        \GPBMetadata\Temporal\Api\Nexus\V1\Message::initOnce();
        parent::__construct($data);
    }

    /**
     * Name of service to start the operation in.
     *
     * Generated from protobuf field <code>string service = 1 [json_name = "service"];</code>
     * @return string
     */
    public function getService()
    {
        return $this->service;
    }

    /**
     * Name of service to start the operation in.
     *
     * Generated from protobuf field <code>string service = 1 [json_name = "service"];</code>
     * @param string $var
     * @return $this
     */
    public function setService(string $var)
    {
        GPBUtil::checkString($var, true);
        $this->service = $var;

        return $this;
    }

    /**
     * Type of operation to start.
     *
     * Generated from protobuf field <code>string operation = 2 [json_name = "operation"];</code>
     * @return string
     */
    public function getOperation()
    {
        return $this->operation;
    }

    /**
     * Type of operation to start.
     *
     * Generated from protobuf field <code>string operation = 2 [json_name = "operation"];</code>
     * @param string $var
     * @return $this
     */
    public function setOperation(string $var)
    {
        GPBUtil::checkString($var, true);
        $this->operation = $var;

        return $this;
    }

    /**
     * A request ID that can be used as an idempotentency key.
     *
     * Generated from protobuf field <code>string request_id = 3 [json_name = "requestId"];</code>
     * @return string
     */
    public function getRequestId()
    {
        return $this->request_id;
    }

    /**
     * A request ID that can be used as an idempotentency key.
     *
     * Generated from protobuf field <code>string request_id = 3 [json_name = "requestId"];</code>
     * @param string $var
     * @return $this
     */
    public function setRequestId(string $var)
    {
        GPBUtil::checkString($var, true);
        $this->request_id = $var;

        return $this;
    }

    /**
     * Callback URL to call upon completion if the started operation is async.
     *
     * Generated from protobuf field <code>string callback = 4 [json_name = "callback"];</code>
     * @return string
     */
    public function getCallback()
    {
        return $this->callback;
    }

    /**
     * Callback URL to call upon completion if the started operation is async.
     *
     * Generated from protobuf field <code>string callback = 4 [json_name = "callback"];</code>
     * @param string $var
     * @return $this
     */
    public function setCallback(string $var)
    {
        GPBUtil::checkString($var, true);
        $this->callback = $var;

        return $this;
    }

    /**
     * Full request body from the incoming HTTP request.
     *
     * Generated from protobuf field <code>.temporal.api.common.v1.Payload payload = 5 [json_name = "payload"];</code>
     * @return \Temporal\Api\Common\V1\Payload|null
     */
    public function getPayload()
    {
        return $this->payload;
    }

    public function hasPayload()
    {
        return isset($this->payload);
    }

    public function clearPayload()
    {
        unset($this->payload);
    }

    /**
     * Full request body from the incoming HTTP request.
     *
     * Generated from protobuf field <code>.temporal.api.common.v1.Payload payload = 5 [json_name = "payload"];</code>
     * @param \Temporal\Api\Common\V1\Payload $var
     * @return $this
     */
    public function setPayload(\Temporal\Api\Common\V1\Payload|null $var)
    {
        $this->payload = $var;

        return $this;
    }

    /**
     * Header that is expected to be attached to the callback request when the operation completes.
     *
     * Generated from protobuf field <code>map<string, string> callback_header = 6 [json_name = "callbackHeader"];</code>
     * @return \Google\Protobuf\Internal\MapField
     */
    public function getCallbackHeader()
    {
        return $this->callback_header;
    }

    /**
     * Header that is expected to be attached to the callback request when the operation completes.
     *
     * Generated from protobuf field <code>map<string, string> callback_header = 6 [json_name = "callbackHeader"];</code>
     * @param array|\Google\Protobuf\Internal\MapField $var
     * @return $this
     */
    public function setCallbackHeader(array|\Google\Protobuf\Internal\MapField $var)
    {
        $arr = GPBUtil::checkMapField($var, \Google\Protobuf\Internal\GPBType::STRING, \Google\Protobuf\Internal\GPBType::STRING);
        $this->callback_header = $arr;

        return $this;
    }

    /**
     * Links contain caller information and can be attached to the operations started by the handler.
     *
     * Generated from protobuf field <code>repeated .temporal.api.nexus.v1.Link links = 7 [json_name = "links"];</code>
     * @return RepeatedField<\Temporal\Api\Nexus\V1\Link>
     */
    public function getLinks()
    {
        return $this->links;
    }

    /**
     * Links contain caller information and can be attached to the operations started by the handler.
     *
     * Generated from protobuf field <code>repeated .temporal.api.nexus.v1.Link links = 7 [json_name = "links"];</code>
     * @param \Temporal\Api\Nexus\V1\Link[] $var
     * @return $this
     */
    public function setLinks(array|RepeatedField $var)
    {
        $arr = GPBUtil::checkRepeatedField($var, \Google\Protobuf\Internal\GPBType::MESSAGE, \Temporal\Api\Nexus\V1\Link::class);
        $this->links = $arr;

        return $this;
    }

}

