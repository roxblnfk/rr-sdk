<?php
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# NO CHECKED-IN PROTOBUF GENCODE
# source: temporal/api/taskqueue/v1/message.proto

namespace Temporal\Api\Taskqueue\V1;

use Google\Protobuf\Internal\GPBType;
use Google\Protobuf\Internal\GPBUtil;
use Google\Protobuf\RepeatedField;

/**
 * Deprecated. Use `InternalTaskQueueStatus`. This is kept until `DescribeTaskQueue` supports legacy behavior.
 *
 * Generated from protobuf message <code>temporal.api.taskqueue.v1.TaskQueueStatus</code>
 */
class TaskQueueStatus extends \Google\Protobuf\Internal\Message
{
    /**
     * Generated from protobuf field <code>int64 backlog_count_hint = 1 [json_name = "backlogCountHint"];</code>
     */
    protected $backlog_count_hint = 0;
    /**
     * Generated from protobuf field <code>int64 read_level = 2 [json_name = "readLevel"];</code>
     */
    protected $read_level = 0;
    /**
     * Generated from protobuf field <code>int64 ack_level = 3 [json_name = "ackLevel"];</code>
     */
    protected $ack_level = 0;
    /**
     * Generated from protobuf field <code>double rate_per_second = 4 [json_name = "ratePerSecond"];</code>
     */
    protected $rate_per_second = 0.0;
    /**
     * Generated from protobuf field <code>.temporal.api.taskqueue.v1.TaskIdBlock task_id_block = 5 [json_name = "taskIdBlock"];</code>
     */
    protected $task_id_block = null;

    /**
     * Constructor.
     *
     * @param array $data {
     *     Optional. Data for populating the Message object.
     *
     *     @type int|string $backlog_count_hint
     *     @type int|string $read_level
     *     @type int|string $ack_level
     *     @type float $rate_per_second
     *     @type \Temporal\Api\Taskqueue\V1\TaskIdBlock $task_id_block
     * }
     */
    public function __construct($data = NULL) {
        \GPBMetadata\Temporal\Api\Taskqueue\V1\Message::initOnce();
        parent::__construct($data);
    }

    /**
     * Generated from protobuf field <code>int64 backlog_count_hint = 1 [json_name = "backlogCountHint"];</code>
     * @return int|string
     */
    public function getBacklogCountHint()
    {
        return $this->backlog_count_hint;
    }

    /**
     * Generated from protobuf field <code>int64 backlog_count_hint = 1 [json_name = "backlogCountHint"];</code>
     * @param int|string $var
     * @return $this
     */
    public function setBacklogCountHint(int|string $var)
    {
        GPBUtil::checkInt64($var);
        $this->backlog_count_hint = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>int64 read_level = 2 [json_name = "readLevel"];</code>
     * @return int|string
     */
    public function getReadLevel()
    {
        return $this->read_level;
    }

    /**
     * Generated from protobuf field <code>int64 read_level = 2 [json_name = "readLevel"];</code>
     * @param int|string $var
     * @return $this
     */
    public function setReadLevel(int|string $var)
    {
        GPBUtil::checkInt64($var);
        $this->read_level = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>int64 ack_level = 3 [json_name = "ackLevel"];</code>
     * @return int|string
     */
    public function getAckLevel()
    {
        return $this->ack_level;
    }

    /**
     * Generated from protobuf field <code>int64 ack_level = 3 [json_name = "ackLevel"];</code>
     * @param int|string $var
     * @return $this
     */
    public function setAckLevel(int|string $var)
    {
        GPBUtil::checkInt64($var);
        $this->ack_level = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>double rate_per_second = 4 [json_name = "ratePerSecond"];</code>
     * @return float
     */
    public function getRatePerSecond()
    {
        return $this->rate_per_second;
    }

    /**
     * Generated from protobuf field <code>double rate_per_second = 4 [json_name = "ratePerSecond"];</code>
     * @param float $var
     * @return $this
     */
    public function setRatePerSecond(float $var)
    {
        $this->rate_per_second = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>.temporal.api.taskqueue.v1.TaskIdBlock task_id_block = 5 [json_name = "taskIdBlock"];</code>
     * @return \Temporal\Api\Taskqueue\V1\TaskIdBlock|null
     */
    public function getTaskIdBlock()
    {
        return $this->task_id_block;
    }

    public function hasTaskIdBlock()
    {
        return isset($this->task_id_block);
    }

    public function clearTaskIdBlock()
    {
        unset($this->task_id_block);
    }

    /**
     * Generated from protobuf field <code>.temporal.api.taskqueue.v1.TaskIdBlock task_id_block = 5 [json_name = "taskIdBlock"];</code>
     * @param \Temporal\Api\Taskqueue\V1\TaskIdBlock $var
     * @return $this
     */
    public function setTaskIdBlock(\Temporal\Api\Taskqueue\V1\TaskIdBlock|null $var)
    {
        $this->task_id_block = $var;

        return $this;
    }

}

