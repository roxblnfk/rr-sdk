<?php
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# NO CHECKED-IN PROTOBUF GENCODE
# source: temporal/api/workflow/v1/message.proto

namespace Temporal\Api\Workflow\V1;

use Google\Protobuf\Internal\GPBType;
use Google\Protobuf\Internal\GPBUtil;
use Google\Protobuf\RepeatedField;

/**
 * Generated from protobuf message <code>temporal.api.workflow.v1.PendingChildExecutionInfo</code>
 */
class PendingChildExecutionInfo extends \Google\Protobuf\Internal\Message
{
    /**
     * Generated from protobuf field <code>string workflow_id = 1 [json_name = "workflowId"];</code>
     */
    protected $workflow_id = '';
    /**
     * Generated from protobuf field <code>string run_id = 2 [json_name = "runId"];</code>
     */
    protected $run_id = '';
    /**
     * Generated from protobuf field <code>string workflow_type_name = 3 [json_name = "workflowTypeName"];</code>
     */
    protected $workflow_type_name = '';
    /**
     * Generated from protobuf field <code>int64 initiated_id = 4 [json_name = "initiatedId"];</code>
     */
    protected $initiated_id = 0;
    /**
     * Default: PARENT_CLOSE_POLICY_TERMINATE.
     *
     * Generated from protobuf field <code>.temporal.api.enums.v1.ParentClosePolicy parent_close_policy = 5 [json_name = "parentClosePolicy"];</code>
     */
    protected $parent_close_policy = 0;

    /**
     * Constructor.
     *
     * @param array $data {
     *     Optional. Data for populating the Message object.
     *
     *     @type string $workflow_id
     *     @type string $run_id
     *     @type string $workflow_type_name
     *     @type int|string $initiated_id
     *     @type int $parent_close_policy
     *           Default: PARENT_CLOSE_POLICY_TERMINATE.
     * }
     */
    public function __construct($data = NULL) {
        \GPBMetadata\Temporal\Api\Workflow\V1\Message::initOnce();
        parent::__construct($data);
    }

    /**
     * Generated from protobuf field <code>string workflow_id = 1 [json_name = "workflowId"];</code>
     * @return string
     */
    public function getWorkflowId()
    {
        return $this->workflow_id;
    }

    /**
     * Generated from protobuf field <code>string workflow_id = 1 [json_name = "workflowId"];</code>
     * @param string $var
     * @return $this
     */
    public function setWorkflowId(string $var)
    {
        GPBUtil::checkString($var, true);
        $this->workflow_id = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>string run_id = 2 [json_name = "runId"];</code>
     * @return string
     */
    public function getRunId()
    {
        return $this->run_id;
    }

    /**
     * Generated from protobuf field <code>string run_id = 2 [json_name = "runId"];</code>
     * @param string $var
     * @return $this
     */
    public function setRunId(string $var)
    {
        GPBUtil::checkString($var, true);
        $this->run_id = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>string workflow_type_name = 3 [json_name = "workflowTypeName"];</code>
     * @return string
     */
    public function getWorkflowTypeName()
    {
        return $this->workflow_type_name;
    }

    /**
     * Generated from protobuf field <code>string workflow_type_name = 3 [json_name = "workflowTypeName"];</code>
     * @param string $var
     * @return $this
     */
    public function setWorkflowTypeName(string $var)
    {
        GPBUtil::checkString($var, true);
        $this->workflow_type_name = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>int64 initiated_id = 4 [json_name = "initiatedId"];</code>
     * @return int|string
     */
    public function getInitiatedId()
    {
        return $this->initiated_id;
    }

    /**
     * Generated from protobuf field <code>int64 initiated_id = 4 [json_name = "initiatedId"];</code>
     * @param int|string $var
     * @return $this
     */
    public function setInitiatedId(int|string $var)
    {
        GPBUtil::checkInt64($var);
        $this->initiated_id = $var;

        return $this;
    }

    /**
     * Default: PARENT_CLOSE_POLICY_TERMINATE.
     *
     * Generated from protobuf field <code>.temporal.api.enums.v1.ParentClosePolicy parent_close_policy = 5 [json_name = "parentClosePolicy"];</code>
     * @return int one of the values in {@see \Temporal\Api\Enums\V1\ParentClosePolicy}
     */
    public function getParentClosePolicy()
    {
        return $this->parent_close_policy;
    }

    /**
     * Default: PARENT_CLOSE_POLICY_TERMINATE.
     *
     * Generated from protobuf field <code>.temporal.api.enums.v1.ParentClosePolicy parent_close_policy = 5 [json_name = "parentClosePolicy"];</code>
     * @param int $var one of the values in {@see \Temporal\Api\Enums\V1\ParentClosePolicy}
     * @return $this
     */
    public function setParentClosePolicy(int $var)
    {
        GPBUtil::checkEnum($var, \Temporal\Api\Enums\V1\ParentClosePolicy::class);
        $this->parent_close_policy = $var;

        return $this;
    }

}

