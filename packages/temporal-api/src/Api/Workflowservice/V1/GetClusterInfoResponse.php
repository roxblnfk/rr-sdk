<?php
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# NO CHECKED-IN PROTOBUF GENCODE
# source: temporal/api/workflowservice/v1/request_response.proto

namespace Temporal\Api\Workflowservice\V1;

use Google\Protobuf\Internal\GPBType;
use Google\Protobuf\Internal\GPBUtil;
use Google\Protobuf\RepeatedField;

/**
 * GetClusterInfoResponse contains information about Temporal cluster.
 *
 * Generated from protobuf message <code>temporal.api.workflowservice.v1.GetClusterInfoResponse</code>
 */
class GetClusterInfoResponse extends \Google\Protobuf\Internal\Message
{
    /**
     * Key is client name i.e "temporal-go", "temporal-java", or "temporal-cli".
     * Value is ranges of supported versions of this client i.e ">1.1.1 <=1.4.0 || ^5.0.0".
     *
     * Generated from protobuf field <code>map<string, string> supported_clients = 1 [json_name = "supportedClients"];</code>
     */
    private $supported_clients;
    /**
     * Generated from protobuf field <code>string server_version = 2 [json_name = "serverVersion"];</code>
     */
    protected $server_version = '';
    /**
     * Generated from protobuf field <code>string cluster_id = 3 [json_name = "clusterId"];</code>
     */
    protected $cluster_id = '';
    /**
     * Generated from protobuf field <code>.temporal.api.version.v1.VersionInfo version_info = 4 [json_name = "versionInfo"];</code>
     */
    protected $version_info = null;
    /**
     * Generated from protobuf field <code>string cluster_name = 5 [json_name = "clusterName"];</code>
     */
    protected $cluster_name = '';
    /**
     * Generated from protobuf field <code>int32 history_shard_count = 6 [json_name = "historyShardCount"];</code>
     */
    protected $history_shard_count = 0;
    /**
     * Generated from protobuf field <code>string persistence_store = 7 [json_name = "persistenceStore"];</code>
     */
    protected $persistence_store = '';
    /**
     * Generated from protobuf field <code>string visibility_store = 8 [json_name = "visibilityStore"];</code>
     */
    protected $visibility_store = '';
    /**
     * Generated from protobuf field <code>int64 initial_failover_version = 9 [json_name = "initialFailoverVersion"];</code>
     */
    protected $initial_failover_version = 0;
    /**
     * Generated from protobuf field <code>int64 failover_version_increment = 10 [json_name = "failoverVersionIncrement"];</code>
     */
    protected $failover_version_increment = 0;

    /**
     * Constructor.
     *
     * @param array $data {
     *     Optional. Data for populating the Message object.
     *
     *     @type array|\Google\Protobuf\Internal\MapField $supported_clients
     *           Key is client name i.e "temporal-go", "temporal-java", or "temporal-cli".
     *           Value is ranges of supported versions of this client i.e ">1.1.1 <=1.4.0 || ^5.0.0".
     *     @type string $server_version
     *     @type string $cluster_id
     *     @type \Temporal\Api\Version\V1\VersionInfo $version_info
     *     @type string $cluster_name
     *     @type int $history_shard_count
     *     @type string $persistence_store
     *     @type string $visibility_store
     *     @type int|string $initial_failover_version
     *     @type int|string $failover_version_increment
     * }
     */
    public function __construct($data = NULL) {
        \GPBMetadata\Temporal\Api\Workflowservice\V1\RequestResponse::initOnce();
        parent::__construct($data);
    }

    /**
     * Key is client name i.e "temporal-go", "temporal-java", or "temporal-cli".
     * Value is ranges of supported versions of this client i.e ">1.1.1 <=1.4.0 || ^5.0.0".
     *
     * Generated from protobuf field <code>map<string, string> supported_clients = 1 [json_name = "supportedClients"];</code>
     * @return \Google\Protobuf\Internal\MapField
     */
    public function getSupportedClients()
    {
        return $this->supported_clients;
    }

    /**
     * Key is client name i.e "temporal-go", "temporal-java", or "temporal-cli".
     * Value is ranges of supported versions of this client i.e ">1.1.1 <=1.4.0 || ^5.0.0".
     *
     * Generated from protobuf field <code>map<string, string> supported_clients = 1 [json_name = "supportedClients"];</code>
     * @param array|\Google\Protobuf\Internal\MapField $var
     * @return $this
     */
    public function setSupportedClients(array|\Google\Protobuf\Internal\MapField $var)
    {
        $arr = GPBUtil::checkMapField($var, \Google\Protobuf\Internal\GPBType::STRING, \Google\Protobuf\Internal\GPBType::STRING);
        $this->supported_clients = $arr;

        return $this;
    }

    /**
     * Generated from protobuf field <code>string server_version = 2 [json_name = "serverVersion"];</code>
     * @return string
     */
    public function getServerVersion()
    {
        return $this->server_version;
    }

    /**
     * Generated from protobuf field <code>string server_version = 2 [json_name = "serverVersion"];</code>
     * @param string $var
     * @return $this
     */
    public function setServerVersion(string $var)
    {
        GPBUtil::checkString($var, true);
        $this->server_version = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>string cluster_id = 3 [json_name = "clusterId"];</code>
     * @return string
     */
    public function getClusterId()
    {
        return $this->cluster_id;
    }

    /**
     * Generated from protobuf field <code>string cluster_id = 3 [json_name = "clusterId"];</code>
     * @param string $var
     * @return $this
     */
    public function setClusterId(string $var)
    {
        GPBUtil::checkString($var, true);
        $this->cluster_id = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>.temporal.api.version.v1.VersionInfo version_info = 4 [json_name = "versionInfo"];</code>
     * @return \Temporal\Api\Version\V1\VersionInfo|null
     */
    public function getVersionInfo()
    {
        return $this->version_info;
    }

    public function hasVersionInfo()
    {
        return isset($this->version_info);
    }

    public function clearVersionInfo()
    {
        unset($this->version_info);
    }

    /**
     * Generated from protobuf field <code>.temporal.api.version.v1.VersionInfo version_info = 4 [json_name = "versionInfo"];</code>
     * @param \Temporal\Api\Version\V1\VersionInfo $var
     * @return $this
     */
    public function setVersionInfo(\Temporal\Api\Version\V1\VersionInfo|null $var)
    {
        $this->version_info = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>string cluster_name = 5 [json_name = "clusterName"];</code>
     * @return string
     */
    public function getClusterName()
    {
        return $this->cluster_name;
    }

    /**
     * Generated from protobuf field <code>string cluster_name = 5 [json_name = "clusterName"];</code>
     * @param string $var
     * @return $this
     */
    public function setClusterName(string $var)
    {
        GPBUtil::checkString($var, true);
        $this->cluster_name = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>int32 history_shard_count = 6 [json_name = "historyShardCount"];</code>
     * @return int
     */
    public function getHistoryShardCount()
    {
        return $this->history_shard_count;
    }

    /**
     * Generated from protobuf field <code>int32 history_shard_count = 6 [json_name = "historyShardCount"];</code>
     * @param int $var
     * @return $this
     */
    public function setHistoryShardCount(int $var)
    {
        GPBUtil::checkInt32($var);
        $this->history_shard_count = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>string persistence_store = 7 [json_name = "persistenceStore"];</code>
     * @return string
     */
    public function getPersistenceStore()
    {
        return $this->persistence_store;
    }

    /**
     * Generated from protobuf field <code>string persistence_store = 7 [json_name = "persistenceStore"];</code>
     * @param string $var
     * @return $this
     */
    public function setPersistenceStore(string $var)
    {
        GPBUtil::checkString($var, true);
        $this->persistence_store = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>string visibility_store = 8 [json_name = "visibilityStore"];</code>
     * @return string
     */
    public function getVisibilityStore()
    {
        return $this->visibility_store;
    }

    /**
     * Generated from protobuf field <code>string visibility_store = 8 [json_name = "visibilityStore"];</code>
     * @param string $var
     * @return $this
     */
    public function setVisibilityStore(string $var)
    {
        GPBUtil::checkString($var, true);
        $this->visibility_store = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>int64 initial_failover_version = 9 [json_name = "initialFailoverVersion"];</code>
     * @return int|string
     */
    public function getInitialFailoverVersion()
    {
        return $this->initial_failover_version;
    }

    /**
     * Generated from protobuf field <code>int64 initial_failover_version = 9 [json_name = "initialFailoverVersion"];</code>
     * @param int|string $var
     * @return $this
     */
    public function setInitialFailoverVersion(int|string $var)
    {
        GPBUtil::checkInt64($var);
        $this->initial_failover_version = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>int64 failover_version_increment = 10 [json_name = "failoverVersionIncrement"];</code>
     * @return int|string
     */
    public function getFailoverVersionIncrement()
    {
        return $this->failover_version_increment;
    }

    /**
     * Generated from protobuf field <code>int64 failover_version_increment = 10 [json_name = "failoverVersionIncrement"];</code>
     * @param int|string $var
     * @return $this
     */
    public function setFailoverVersionIncrement(int|string $var)
    {
        GPBUtil::checkInt64($var);
        $this->failover_version_increment = $var;

        return $this;
    }

}

